...
package com.omer_dogan.turkanime
...
import android.util.Base64
    ...
    private fun iframe2AesLink(iframe: String): String? {
        ...
        val decrypted = try {
            val helperClass = Class.forName("com.lagradost.cloudstream3.extractors.helper.AesHelper")
            val method = helperClass.getMethod("cryptoAESHandler", String::class.java, ByteArray::class.java, Boolean::class.javaPrimitiveType)
            method.invoke(null, aesData, aesKey.toByteArray(), false) as? String
        } catch (e: Exception) {
            null
        }
        val aesLink = decrypted?.replace("\\", "") ?: throw ErrorLoadingException("failed to decrypt")
