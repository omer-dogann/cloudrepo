...
package com.omer_dogan.dizilla
...
import javax.crypto.spec.SecretKeySpec
    ...
    override suspend fun loadLinks(
        ...
        val objectMapper = ObjectMapper().registerModule(KotlinModule.Builder().build())
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
        val secureData = objectMapper.readTree(script).get("props").get("pageProps").get("secureData")?.asText() ?: return false
        val decodedData = decryptDizillaResponse(secureData) ?: return false
        val resultNode = objectMapper.readTree(decodedData).get("RelatedResults")
            ?.get("getEpisodeSources")?.get("result") ?: return false

        var loadedAny = false
        if (resultNode.isArray) {
            for (item in resultNode) {
                val sourceContent = item.get("source_content")?.asText() ?: continue
                val iframe = fixUrlNull(Jsoup.parse(sourceContent).select("iframe").attr("src")) ?: continue
