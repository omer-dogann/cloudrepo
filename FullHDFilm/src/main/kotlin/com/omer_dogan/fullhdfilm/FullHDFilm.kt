...
class FullHDFilm : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val url = if (page == 1) request.data.removeSuffix("/page").removeSuffix("page") + "/" else "${request.data}/${page}/"
        val document = app.get(url).document
        val home     = document.select("div.movie_box, div.film").mapNotNull { it.toMainPageResult() }
