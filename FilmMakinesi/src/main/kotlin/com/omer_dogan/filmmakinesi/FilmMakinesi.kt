...
package com.omer_dogan.filmmakinesi
...
class FilmMakinesi : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home     = if (request.data.contains("/film-izle/")) {
            document.select("section#film_posts article, div.film, div.post, article").mapNotNull { it.toSearchResult() }
        } else {
            document.select("section#film_posts div.tooltip, div.film, article, div.post").mapNotNull { it.toSearchResult() }
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select("section#film_posts article, div.film, div.post, article").mapNotNull { it.toSearchResult() }
    }
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val iframes = document.select("div.player-div iframe, div.embed-inside iframe, iframe.player-iframe, div.video-container iframe")
        for (element in iframes) {
            val iframe = fixUrlNull(element.attr("src").ifEmpty { element.attr("data-src") }) ?: continue
