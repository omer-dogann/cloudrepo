...
package com.omer_dogan.hdfilmcehennemi
...
import org.jsoup.nodes.Element
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        if (!doc.toString().contains("Sayfa Bulunamadı")) {
            val aa: HDFC? = tryParseJson<HDFC>(doc.text) ?: try { objectMapper.readValue(doc.text, HDFC::class.java) } catch(e: Exception) { null }
            val htmlStr = aa?.html ?: doc.text
            val document = Jsoup.parse(htmlStr)

            home = document.select("a, div.poster, div.movie").mapNotNull { it.toSearchResult() }
