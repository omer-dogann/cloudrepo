...
class TrAnimaci : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "${mainUrl}/yeni-eklenenler?page=" to "Yeni Eklenenler",
    )

    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val doc = app.get(request.data + page).document
        val home = doc.select("div.item, article, div.film, div.anime-card, div.poster").mapNotNull { element ->
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.item, article, div.film, div.anime-card, div.poster").mapNotNull { element ->
            val title = element.selectFirst("h2, h3, a.title, div.title, span.title")?.text()?.trim() ?: return@mapNotNull null
            val href = element.selectFirst("a")?.attr("href") ?: return@mapNotNull null
            val poster = element.selectFirst("img")?.attr("src") ?: element.selectFirst("img")?.attr("data-src")
